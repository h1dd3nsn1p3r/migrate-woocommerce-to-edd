...
//		}
	...
	if( $product->product_type == 'variable' ) {
		...
		$edd_variations[0]['license_limit'] = '1';
		$edd_variations[1]['license_limit'] = '10';
		$edd_variations[2]['license_limit'] = '999999';
	...
	if( edd_has_variable_prices( $edd_product_id ) ) {
		update_post_meta( $edd_product_id, $edd_sl_slug, true );
	} else {
		update_post_meta( $edd_product_id, $edd_sl_slug, false );
	}
	...
	$edd_sl_enabled = get_post_meta( $edd_product_id, $edd_sl_slug, true );
	...
	if( $edd_sl_enabled ) {
		...
		update_post_meta( $edd_product_id, $edd_activation_limit_slug, (int) '1' );
		...
		// Software Version
		// $wc_api_version_slug = '_api_new_version';
		// WooSL product version
		$wc_product_ver = get_post_meta( $p->ID, '_sl_new_version', true );
		$edd_sl_version_slug = '_edd_sl_version';
		update_post_meta( $edd_product_id, $edd_sl_version_slug, $wc_product_ver );
		$temp_log_str = "\nWC Product Version : " . $wc_product_ver . " migrated ...\n";
		$log_str .= $temp_log_str;
		echo $temp_log_str;
		...
		$edd_dl_is_lifetime = "edd_sl_download_lifetime";
		update_post_meta( $edd_product_id, $edd_dl_is_lifetime, 0 );
		$edd_dl_expiry_unit_slug = "_edd_sl_exp_unit";
		update_post_meta( $edd_product_id, $edd_dl_expiry_unit_slug, 'years' );
		$edd_dl_expiry_length_slug = "_edd_sl_exp_length";
		update_post_meta( $edd_product_id, $edd_dl_expiry_length_slug, (int) '1' );
	...
	$edd_dl_discount_percentage = "_edd_sl_renewal_discount";
	update_post_meta( $edd_product_id, $edd_dl_discount_percentage, (int) '30' );
...
foreach( $wc_order_list as $o ) {
	...
	foreach( $wc_items as $item ) {
		...
		// Get Purchased Variation of Order
		$_wc_variation_id = $product->get_variation_id();
		$_wc_parent_id = $product->get_parent_id();
		$_wc_license_key = '';
		if( $_wc_parent_id != 0 ) {
			$_wc_variation = new WC_Product_Variation( $_wc_variation_id );
			$_wc_variation_attributes = $_wc_variation->get_variation_attributes();
			$_wc_license_key = $_wc_variation_attributes['attribute_license'];
		}
		...
		// Set Price ID for Purchased Variation Order
		if( $_wc_license_key ) {
			switch ($_wc_license_key) {
				
				case 'Multiple_License':
					$item_number['options'] = array( 'price_id' => 1 );
					break;

				case 'Developer_License':
					$item_number['options'] = array( 'price_id' => 2 );
					break;
				
				default:
					$item_number['options'] = array( 'price_id' => 0 );
					break;
			}
		} else {
			$item_number['options'] = array();
		}
